#!/usr/bin/env python
"""
make_rose_task_names_mohc_hist_ice.py
...
def main(args):
    ...
    orca1 = DataRequest.objects.filter(
        climate_model__short_name='HadGEM3-GC31-LL',
        experiment__short_name__in=['hist-1950', 'control-1950', 'highres-future', 'spinup-1950'],
        rip_code__in=[f'r1i{i}p1f1' for i in range(1,9)],
        variable_request__table_name__in=['SImon', 'SIday', 'PrimSIday'], 
        datafile__isnull=False
    ).distinct()

    orca025 = DataRequest.objects.filter(
        climate_model__short_name__in=['HadGEM3-GC31-MM', 'HadGEM3-GC31-HM'],
        experiment__short_name__in=['hist-1950', 'control-1950', 'highres-future', 'spinup-1950'],
    ...
    task_name_list = [
        ...
        for dr in all_tasks.order_by('climate_model__short_name',
